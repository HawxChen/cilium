...
func NewDaemon(c *Config) (*Daemon, error) {
	...
	case kvstore.Etcd:
		if c.EtcdCfgPath != "" || c.EtcdConfig != nil {
			...
			return nil, fmt.Errorf("Invalid configuration for etcd provided; please specify an etcd configuration path with --etcd-config-path and an etcd agent address with --etcd")
		}
	case kvstore.Local:
		log.Infof("Using local storage as key-value store")
		kvClient = kvstore.NewLocalClient()
	default:
		return nil, fmt.Errorf("Invalid key-value store type provided; please use one of the following: %s, %s, %s", kvstore.Consul, kvstore.Etcd, kvstore.Local)
